...
if (!$isAdmin && !$isLandlord) {
    echo '<main class="max-w-4xl mx-auto p-4">
...
if ($isAdmin) {
    $stmt = $pdo->query("
        SELECT p.*, 
               (SELECT filename FROM post_images WHERE post_id = p.id LIMIT 1) AS thumbnail,
...
<main class="max-w-6xl mx-auto p-4">
    ...
    <?php if (!$posts): ?>
        <p class="text-gray-600">Không có bài đăng nào.</p>
