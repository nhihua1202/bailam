...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        // MÀN HÌNH THÀNH CÔNG CÓ NÚT TRANG CHỦ
...
<style>
    body { background-color: #FFF0F5 !important; } /* Nền hồng cho toàn trang */
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl border">
        ...
        <form method="POST" class="grid gap-4">
            <div>
                ...
                <input name="name" value="<?= esc($data['name']) ?>" class="mt-1 border p-2 w-full rounded focus:ring-2 focus:ring-green-400 outline-none">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="dob" value="<?= esc($data['dob']) ?>" class="mt-1 border p-2 w-full rounded outline-none">
                </div>
                <div>
                    <label class="text-sm font-semibold text-gray-700">CCCD *</label>
                    <input name="cccd" maxlength="12" value="<?= esc($data['cccd']) ?>" class="mt-1 border p-2 w-full rounded outline-none">
                </div>
            </div>

            <div>
                <label class="text-sm font-semibold text-gray-700">Quê quán *</label>
                <input name="address" value="<?= esc($data['address']) ?>" class="mt-1 border p-2 w-full rounded outline-none">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="text-sm font-semibold text-gray-700">Số điện thoại *</label>
                    <input name="phone" value="<?= esc($data['phone']) ?>" class="mt-1 border p-2 w-full rounded outline-none">
                </div>
                <div>
                    <label class="text-sm font-semibold text-gray-700">Email *</label>
                    <input type="email" name="email" value="<?= esc($data['email']) ?>" class="mt-1 border p-2 w-full rounded outline-none">
                </div>
            </div>
