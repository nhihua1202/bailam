...
/* ================= ẢNH QUẢNG CÁO 2 BÊN ================= */
$adsLeft = [
    'https://via.placeholder.com/160x600?text=QC+Trai+1',
    'https://via.placeholder.com/160x600?text=QC+Trai+2'
];

$adsRight = [
    'https://via.placeholder.com/160x600?text=QC+Phai+1',
    'https://via.placeholder.com/160x600?text=QC+Phai+2'
];
...
<head>
    ...
    <style>
        ...
        /* Khối bao quanh để cố định banner theo nội dung */
        .page-wrapper {
            position: relative;
            max-width: 1280px; /* Khớp với max-w-7xl */
            margin: 0 auto;
        }

        /* Định dạng banner 2 bên */
        .side-banner {
            position: absolute;
            top: 20px;
            width: 160px;
        ...
        /* Chỉ hiện khi màn hình > 1600px để không che chữ */
    ...
    <form method="get" class="mx-auto w-[95%] max-w-5xl">
        <div class="bg-white/95 backdrop-blur-md border border-gray-100 rounded-full shadow-[0_15px_45px_rgba(0,0,0,0.1)] p-1.5 flex items-center transition-all hover:shadow-2xl">
            <div class="flex-1 flex items-center pl-6">
                ...
                <input type="text" name="q" value="<?=esc($q)?>" placeholder="Nhập khu vực, tên đường..." class="w-full bg-transparent border-none focus:ring-0 text-sm py-2 text-gray-700 outline-none">
            </div>
            <div class="flex items-center">
                <div class="hidden md:flex items-center px-4 border-l border-gray-100">
                    <select name="type" class="bg-transparent border-none text-[13px] font-semibold text-gray-600 focus:ring-0 cursor-pointer py-1">
                ...
                <div class="hidden lg:flex items-center px-4 border-l border-gray-100">
                    <select name="khu_vuc" class="bg-transparent border-none text-[13px] font-semibold text-gray-600 focus:ring-0 cursor-pointer py-1">
                ...
                <div class="hidden sm:flex items-center px-4 border-l border-gray-100">
                    <select name="price_range" class="bg-transparent border-none text-[13px] font-semibold text-gray-600 focus:ring-0 cursor-pointer py-1">
            ...
            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white h-11 px-7 rounded-full font-bold ml-2">Tìm kiếm</button>
        </div>
        <div class="flex justify-center mt-4">
            <a href="index.php" class="flex items-center gap-2 px-4 py-1.5 bg-white/80 rounded-full border border-gray-100 shadow-sm text-[11px] font-bold text-gray-500 uppercase tracking-widest hover:text-red-500 transition-all">
                Làm mới bộ lọc
            </a>
        </div>
    </form>
        ...
        foreach($rooms as $p){
            ...
            <a href='post.php?id={$p['id']}' class='group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all overflow-hidden border border-gray-100'>
                <div class='relative overflow-hidden'>
                    ...
                    <div class='absolute top-3 right-3 bg-white/90 px-3 py-1 rounded-full text-xs font-bold text-red-500'>$m triệu/tháng</div>
                </div>
                <div class='p-5'>
